module.exports = {
  siteTitle: 'Jp Valery - Photographer documenting spaces and people',
  siteTitleAlt: 'Jp Valery - Photographer documenting spaces and people', // This allows an alternative site title for SEO schema.
  publisher: 'Jp Valery', // Organization name used for SEO schema
  siteDescription:
    'Selected shots from a photographer relentlessly documenting and exploring spaces and people.',
  siteUrl: 'http://jpvalery.photo', // Site domain. Do not include a trailing slash! If you wish to use a path prefix you can read more about that here: https://www.gatsbyjs.org/docs/path-prefix/
  author: 'Jp Valery', // Author for RSS author segment and SEO schema
  authorUrl: 'https://jpvalery.photo', // URL used for author and publisher schema, can be a social profile or other personal site
  userTwitter: '@jpvalery', // Change for Twitter Cards
  shortTitle: 'Jp Valery', // Used for App manifest e.g. Mobile Home Screen
  shareImage: '/logos/share.jpg', // Open Graph Default Share Image. 1200x1200 is recommended
  shareImageWidth: 900, // Change to the width of your default share image
  shareImageHeight: 504, // Change to the height of your default share image
  siteLogo: '/logos/logo-512.png', // Logo used for SEO, RSS, and App manifest
  backgroundColor: '#101010', // Used for Offline Manifest
  themeColor: '#F5F5F5', // Used for Offline Manifest
  copyright: '© 1992-2019 Jp Valery', // Copyright string for the RSS feed
}
